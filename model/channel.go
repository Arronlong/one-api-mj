...
type Channel struct {
	...
	Group              string  `json:"group" gorm:"type:varchar(100);default:'default'"`
...
func GetAllChannels(startIdx int, num int, selectAll bool) ([]*Channel, error) {
	...
	var err error
	if selectAll {
		err = DB.Order("priority desc, id desc").Find(&channels).Error
	} else {
		err = DB.Order("priority desc, id desc").Limit(num).Offset(startIdx).Omit("key").Find(&channels).Error
	}
	return channels, err
}
