...
1. 添加[Midjourney-Proxy](https://github.com/novicezk/midjourney-proxy)接口的支持：
   ...
   + 默认开启MJ图片代理（需要本系统能访问到图片）
   + 添加关闭本系统的MJ图片代理，改为302到原图片地址
   + 添加设置302时强制启用指定图片代理对图片地址（xxx/attachments...中是xxx部分）进行替换，以解决无法直接访问原图片的问题
...
2. 支持配置镜像以及众多第三方代理服务：
   ...
   + [x] [CloseAI](https://www.closeai-asia.com/r/2412)
